...
var _ = Describe("Proportion", func() {
	Describe("setResourceShare - GPU", func() {
		Context("single queue within quota (sanity)", func() {
			...
			It("gives requested GPUs - no remaining", func() {
				remaining := setResourceShare(2, 0, 1, rs.GpuResource, queues)
			...
			It("gives requested GPUs with remaining", func() {
				remaining := setResourceShare(3, 0, 1, rs.GpuResource, queues)
			...
			It("gives up to deserved when over subscribed", func() {
				remaining := setResourceShare(1, 0, 1, rs.GpuResource, queues)
				Expect(remaining).To(Equal(float64(0)))
				Expect(queues["1"].GPU.FairShare).To(Equal(float64(2)))
			})
			It("gives up to full quota", func() {
				queues["1"].GPU.Request = float64(5)
				remaining := setResourceShare(7, 0, 1, rs.GpuResource, queues)
		...
		Context("single queue over quota (sanity)", func() {
			...
			It("gives requested GPUs no remaining", func() {
				remaining := divideOverQuotaResource(2, 0, 1, queues, rs.GpuResource)
				Expect(remaining).To(Equal(float64(0)))
				Expect(queues["1"].GPU.FairShare).To(Equal(float64(5)))
			})
			It("gives requested GPUs with remaining", func() {
				remaining := divideOverQuotaResource(3, 0, 1, queues, rs.GpuResource)
				Expect(remaining).To(Equal(float64(1)))
				Expect(queues["1"].GPU.FairShare).To(Equal(float64(5)))
			})
			It("doesn't give more GPUs than allowed", func() {
			...
			It("gives remainder fraction", func() {
				remaining := divideOverQuotaResource(0.5, 0, 1, queues, rs.GpuResource)
				Expect(remaining).To(Equal(float64(0)))
			...
			It("gives when deserved is zero", func() {
				...
				remaining := divideOverQuotaResource(6, 0, 1, queues, rs.GpuResource)
				Expect(remaining).To(Equal(float64(1)))
				Expect(queues["1"].GPU.FairShare).To(Equal(float64(5)))
			})
			It("doesn't break when there are zero resources to divide", func() {
				origFairShare := queues["1"].GPU.FairShare
				remaining := divideOverQuotaResource(0, 0, 1, queues, rs.GpuResource)
				Expect(remaining).To(Equal(float64(0)))
			...
			It("doesn't give when over quota weight is zero", func() {
				...
				remaining := divideOverQuotaResource(10, 0, 1, queues, rs.GpuResource)
		...
		Context("two queues", func() {
			...
			DescribeTable("two queues", func(testData testMetadata) {
				...
				remaining := setResourceShare(testData.totalGPUs, 0, 1, rs.GpuResource, queues)
		...
		It("divides the remainder even when using priorities", func() {
			...
			remaining := setResourceShare(5, 0, 1, rs.GpuResource, queues)
	...
	Describe("setResourceShare - CPU", func() {
		Context("one queue", func() {
			...
			It("gives requested CPUs to queue", func() {
				remaining := setResourceShare(3000, 0, 1, rs.CpuResource, queues)
			...
			It("gives deserved in over subscription", func() {
				remaining := setResourceShare(1000, 0, 1, rs.CpuResource, queues)
				Expect(remaining).To(Equal(float64(0)))
				Expect(queues["1"].CPU.FairShare).To(Equal(float64(2000)))
			})
		})
		Context("two queues", func() {
			...
			When("queue 2 has no deserved CPU", func() {
				It("reclaims all resources of queue 2", func() {
					remaining := setResourceShare(totalCPUResources, 0, 1, rs.CpuResource, queues)
					Expect(remaining).To(Equal(float64(0)))
				...
				When("all queues deserve the same fairShare of CPU", func() {
					...
					When("all queues request more than they deserve", func() {
						...
						It("all queues get their deserved fairShare - over quota", func() {
							remaining := setResourceShare(18000, 0, 1, rs.CpuResource, queues)
							Expect(remaining).To(Equal(float64(0)))
	...
	Describe("setResourceShare - memory", func() {
		Context("one queue", func() {
			...
			It("gives requested memory to queue", func() {
				remaining := setResourceShare(3000, 0, 1, rs.MemoryResource, queues)
				Expect(remaining).To(Equal(float64(1000)))
				Expect(queues["1"].Memory.FairShare).To(Equal(float64(2000)))
			})
			It("gives deserved in over subscription", func() {
				remaining := setResourceShare(1000, 0, 1, rs.MemoryResource, queues)
				Expect(remaining).To(Equal(float64(0)))
				Expect(queues["1"].Memory.FairShare).To(Equal(float64(2000)))
			})
		})
		Context("two queues", func() {
			...
			When("queue 2 has no deserved memory", func() {
				It("reclaims all resources of queue 2", func() {
					remaining := setResourceShare(totalMemoryResources, 0, 1, rs.MemoryResource, queues)
					Expect(remaining).To(Equal(float64(0)))
			...
			Context("no pre-allocated resources", func() {
				When("all queues deserve the same fairShare of memory", func() {
					...
					When("all queues request more than they deserve", func() {
						...
						It("all queues get their deserved fairShare - over quota", func() {
							remaining := setResourceShare(18000, 0, 1, rs.MemoryResource, queues)
							Expect(remaining).To(Equal(float64(0)))
		...
		for contextName, contextData := range tests {
			...
			Context(cName, func() {
				for testName, testData := range cData {
					...
					testData := testData
					It(testName, func() {
						SetResourcesShare(testData.totalResources, map[rs.ResourceName]float64{}, 1, testData.queues)
	...
	Describe("SetResourcesShare with historical usage", func() {
		DescribeTable("test cases", func(queues map[common_info.QueueID]*rs.QueueAttributes, totalResources rs.ResourceQuantities, totalUsageCapacity map[rs.ResourceName]float64, expectedFairShares map[common_info.QueueID]rs.ResourceQuantities) {
			for qid, queue := range queues {
				queue.UID = "queue-" + qid
			}
			SetResourcesShare(totalResources, totalUsageCapacity, 1, queues)
			for queueName, expectedFairShare := range expectedFairShares {
				actualFairShare := queues[queueName].GetFairShare()
				for _, resource := range rs.AllResources {
					var expectedResourceShare float64
					var found bool
					if expectedResourceShare, found = expectedFairShare[resource]; !found {
						continue
					}
					Expect(actualFairShare[resource]).To(Equal(expectedResourceShare), "Expected queue %s %s fair share to be %.2f, but got %.2f", queueName, resource, expectedFairShare, actualFairShare[resource])
				}
			}
		},
			Entry("Penalize usage",
				map[common_info.QueueID]*rs.QueueAttributes{
					"1": {
						QueueResourceShare: rs.QueueResourceShare{
							GPU: rs.ResourceShare{
								OverQuotaWeight: 1,
			...
			Entry("Identical usage has no effect",
				map[common_info.QueueID]*rs.QueueAttributes{
					"1": {
						QueueResourceShare: rs.QueueResourceShare{
							GPU: rs.ResourceShare{
			...
			Entry("Sanity check",
				map[common_info.QueueID]*rs.QueueAttributes{
					"1": {
						QueueResourceShare: QueueResourceShareOverrides{}.ResourceShare(),
					},
					"2": {
						QueueResourceShare: QueueResourceShareOverrides{}.ResourceShare(),
					},
				...
				map[common_info.QueueID]rs.ResourceQuantities{
					"1": {
						rs.GpuResource: 50,
					},
					"2": {
			...
			Entry("Sanity check with usage",
				map[common_info.QueueID]*rs.QueueAttributes{
					"1": {
						QueueResourceShare: QueueResourceShareOverrides{}.ResourceShare(),
					},
					"2": {
						QueueResourceShare: QueueResourceShareOverrides{
							GPU: &ResourceShareOverrides{
								AbsoluteUsage: ptr.Float64(3),
							},
						}.ResourceShare(),
					},
				},
