...
var _ = Describe("Proportion", func() {
		...
		for contextName, contextData := range tests {
			...
			Context(cName, func() {
				for testName, testData := range cData {
					...
					testData := testData
					It(testName, func() {
						SetResourcesShare(testData.totalResources, map[rs.ResourceName]float64{}, testData.queues)
	...
	Describe("SetResourcesShare with historical usage", func() {
		DescribeTable("test cases", func(queues map[common_info.QueueID]*rs.QueueAttributes, totalResources rs.ResourceQuantities, totalUsageCapacity map[rs.ResourceName]float64, expectedShare map[common_info.QueueID]*rs.QueueAttributes) {
			SetResourcesShare(totalResources, totalUsageCapacity, queues)
			for queueName, expectedQueue := range expectedShare {
				actualFairShare := queues[queueName].GetFairShare()
				expectedFairShare := expectedQueue.GetFairShare()
				for _, resource := range rs.AllResources {
					Expect(actualFairShare[resource]).To(Equal(expectedFairShare[resource]), "Expected queue %s %s fair share to be %.2f, but got %.2f", queueName, resource, expectedFairShare[resource], actualFairShare[resource])
				}
				// Reset
				queues[queueName].CPU.FairShare = 0
				queues[queueName].Memory.FairShare = 0
				queues[queueName].GPU.FairShare = 0
			}
		},
